// UseDifferentIterators.cpp : Defines the entry point for the console application.
//
...
int main()
{
	...
	//fill_n(front_inserter(v1), 6, 2); // Can't use front_inserter with vector
	...
	//distance = rtwo - two; // Reverse iterator and iterator are different types, can't be used together
	...
	auto it = begin(ca);
	it++;
	//*it = 7; // Values in ca are const
	int i = *it;

	//vector<const int> vc; // Can't make vectors of const elements

	it = find(begin(ca), end(ca), 1);
	it++;
	//*it = 7; // it points to a const int
	//it = cbegin(ca); // Iterators and const iterators are different types
	...
	*it2 = 7;
	...
	auto cit = cbegin(v2);
	cit++;
	//*cit = 7; // cit is a const iterator and can't modify values
